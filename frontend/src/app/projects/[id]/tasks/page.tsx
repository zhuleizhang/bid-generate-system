...
export default function TaskKanbanPage() {
    ...
    return (
      ...
      >
        <Space direction="vertical" size={4} style={{ width: "100%" }}>
          ...
          <Space wrap size={4}>
            <Tag color={TASK_TYPE_COLORS[task.task_type] || "default"}>
          ...
          <Space size={12}>
            <Tooltip title={ASSIGNEE_TYPE_LABELS[task.assignee_type]}>
              <span style={{ fontSize: 12, color: "#888" }}>
                {ASSIGNEE_ICON[task.assignee_type]}
                {" "}
                {task.assignee || ASSIGNEE_TYPE_LABELS[task.assignee_type]}
              </span>
            </Tooltip>
            {reqCount > 0 && (
  ...
  const taskRequirements =
    detailTask?.related_requirement_ids
              ...
              {KANBAN_COLUMNS.map((col) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <Space>
                        <Tag color={STATUS_COLORS[col.key]}>
                          {col.title}
                        </Tag>
              ...
              columns={[
                {
                  ...
                  dataIndex: "title",
                  key: "title",
                  render: (title: string, record: BidTaskDBItem) => (
                    <a
                      onClick={() => {
                ...
                },
                {
                  ...
                  width: 100,
                  render: (t: string) => (
                    <Tag color={TASK_TYPE_COLORS[t] || "default"}>
                ...
                },
                {
                  ...
                  key: "assignee_type",
                  width: 120,
                  render: (a: string, record: BidTaskDBItem) => (
                    <Space size={4}>
                      <Tag color={ASSIGNEE_TYPE_COLORS[a]}>
                        {ASSIGNEE_ICON[a]} {ASSIGNEE_TYPE_LABELS[a]}
                      </Tag>
                      {record.assignee && (
                        <Text type="secondary" style={{ fontSize: 12 }}>
                          {record.assignee}
                ...
                },
                {
                  ...
                  width: 90,
                  render: (ids: string[]) => (
                    <Text type="secondary">{ids?.length || 0} 条</Text>
                  ),
                },
                {
                  title: "操作",
                  key: "actions",
                  width: 160,
                  render: (_: unknown, record: BidTaskDBItem) => (
                    <Space size={4}>
                      ...
                      {record.status !== "in_progress" && (
                        <Button
                          ...
                          type="link"
                          onClick={() =>
                            updateTaskStatus(record.id, "in_progress")
                          }
                        >
                          开始
                        </Button>
          ...
          <div>
            <Descriptions size="small" column={2} style={{ marginBottom: 16 }}>
              <Descriptions.Item label="任务类型">
                <Tag color={TASK_TYPE_COLORS[detailTask.task_type] || "default"}>
                  {TASK_TYPE_LABELS[detailTask.task_type] || detailTask.task_type}
                </Tag>
              </Descriptions.Item>
              <Descriptions.Item label="优先级">
                <Tag color={PRIORITY_COLORS[detailTask.priority]}>
              ...
              <Descriptions.Item label="负责人类型">
                <Tag color={ASSIGNEE_TYPE_COLORS[detailTask.assignee_type]}>
                  {ASSIGNEE_ICON[detailTask.assignee_type]}{" "}
                  {ASSIGNEE_TYPE_LABELS[detailTask.assignee_type]}
                </Tag>
              </Descriptions.Item>
                ...
                {(["pending", "in_progress", "completed"] as const).map(
                  (s) => (
                    <Button
                      key={s}
                      size="small"
                      type={detailTask.status === s ? "primary" : "default"}
                      onClick={() => {
