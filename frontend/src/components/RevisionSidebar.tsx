...
export default function RevisionSidebar({ revisions, onStatusChange, projectId }: Props) {
	...
	const [editingRevision, setEditingRevision] = useState<AIRevision | null>(
		null,
	);
	...
	const [batchConfirmAction, setBatchConfirmAction] = useState<
		'' | 'accept_low_risk' | 'reject_selected'
	>('');
	...
	const lowRiskPendingRevisions = useMemo(
		() =>
			revisions.filter(
				(r) => r.risk_level !== 'high' && r.status === 'pending',
			),
		[revisions],
	);
	...
	const renderBatchToolbar = () => {
		const allSelected =
			...
			selectedIds.size === actionableRevisions.length;
	...
	const renderBatchConfirmModal = () => {
		...
		const count = isAccept
			? lowRiskPendingRevisions.length
			: selectedIds.size;
			...
			<Space size={2} wrap>
				...
				<Tooltip title="标记待确认">
					<Button
						...
						loading={actionLoading === rev.id}
						onClick={() =>
							handleStatusChange(rev.id, 'need_human_confirm')
						}
					/>
				</Tooltip>
			</Space>
