...
export default function UnfinishedPanel({
	items,
	onStatusChange,
	onItemLocate,
}: Props) {
	...
	const filteredItems = useMemo(() => {
		...
		if (filterRisk.length > 0) {
			filtered = filtered.filter((i) =>
				filterRisk.includes(i.risk_level),
			);
		}
	...
	const handleMarkResolved = useCallback(
		async (itemId: string) => {
			...
			try {
				await api.patch(`/api/unfinished/${itemId}/status`, {
					status: 'resolved',
				});
	...
	return (
		<>
					...
					items={riskGroups.map((group) => ({
						...
						label: (
							<Space size={4}>
								<Tag
									color={
										UNFINISHED_RISK_COLORS[group.riskLevel]
									}
								>
									{UNFINISHED_RISK_LABELS[group.riskLevel] ||
										group.riskLevel}
								</Tag>
			...
			<Modal
				...
				cancelText="取消"
				confirmLoading={
					!!(ignoreItem && actionLoading === ignoreItem.id)
				}
